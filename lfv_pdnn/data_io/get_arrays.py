...
def get_sig(
    ...
    for sig in sig_list:
        ...
        if sig_all_array_norm is None:
            ...
            sig_all_array_norm = np.concatenate(
                (sig_all_array_norm, temp_array))
    ...
    for npy in npy_list:
            ...
            try:
                ...
                for feature in (selected_features + [channel, "weight"]):
                    ...
                    if npy_array is None:
                        ...
                        npy_array = np.concatenate(
                            (npy_array, temp_array), axis=1)
            except:
        ...
        if len(cut_features) > 0:
            ...
            assert len(cut_features) == len(cut_values) and len(cut_features) == len(
                cut_types), "cut_features and cut_values and cut_types should have same lenth."
            pass_index = None
            for cut_feature_id, (cut_value, cut_type) in enumerate(zip(cut_values, cut_types)):
            ...
            print(npy, "shape:", npy_array.shape,
                  "total weights:", total_weights, "(after cuts)")
