...
def dump_flat_ntuple_individual(root_path: str,
                                ntuple_name: str,
                                variable_list: list,
                                save_dir: str,
                                save_pre_fix: str,
...
def save_array(array, directory_path, file_name, dump_empty=False):
  ...
  """
    ...
    if array.size == 0:
        if dump_empty:
            logging.warning(
                "Empty array detected! Will save empty array as specified.")
        else:
            logging.warning(
                "Empty array detected! Skipping saving current array to: " +
                save_path)
            return
