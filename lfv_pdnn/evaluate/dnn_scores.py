...
def plot_mva_scores(model_wrapper, config, save_path):
    logging.info("Plotting MVA scores")
    ...
    if config.use_root:
        if plot_utils_root.HAS_ROOT:
            ...
            logging.error("Can't import ROOT, try to use matplotlib as backend.")
    else:
        plot_scores_separate_plt(
            sig_scores_dict,
            sig_weights_dict,
            bkg_scores_dict,
            bkg_weights_dict,
...
def plot_scores_separate_plt(
    ...
    """
    logging.info("Plotting MVA scores with matplotlib backend")
    ...
    if config.log:
        ax.set_yscale("log")
        ax.set_title(config.plot_title + "(log)")
    else:
        ax.set_title(config.plot_title + "(lin)")
